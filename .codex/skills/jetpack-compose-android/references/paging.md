...
Use the versions already pinned by the project unless the task explicitly
includes a dependency upgrade.

### Dependency verification rule

Before suggesting a Paging API, confirm the project dependency version actually
ships it. Check `libs.versions.toml` or `build.gradle.kts` before recommending
an API surface.
