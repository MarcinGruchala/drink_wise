...
The same rule applies to any Android configuration source: include the value
that can change, not just the object that provided it.
...
Always use `collectAsStateWithLifecycle` for `StateFlow` and `SharedFlow` in
Android screen composables. Reserve plain `collectAsState` for local Compose
state sources or carefully reviewed non-lifecycle-aware contexts.
...
Keep `rememberSaveable` at the screen level for small values that truly need
process-death restoration. Use plain `remember` inside list items unless there
is a specific restoration requirement.
