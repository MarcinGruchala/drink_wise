...
### Category 4: Android API Boundaries

Apply these checks when Android framework APIs appear in UI code.

- [ ] `Context`, `Activity`, and `Application` references stay at screen or
  platform boundaries; do not pass them through reusable child composables.
- [ ] `LocalContext.current` is used only for Android integration points
  such as resource access, launching intents, or platform services.
- [ ] User-visible strings use Android string resources instead of hardcoded
  text literals.
