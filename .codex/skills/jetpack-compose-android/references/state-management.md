...
## Android State Persistence Notes

Use `rememberSaveable` for small UI values that should survive Android process
recreation. Keep large objects, lists, and domain models out of the saved-state
bundle.
...
Use `collectAsStateWithLifecycle()` for screen state exposed from ViewModels.
Only use plain `collectAsState()` when lifecycle awareness is intentionally not
needed.
