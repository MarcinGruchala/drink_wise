...
- **You are starting a greenfield Compose navigation surface and want direct
  ownership of the back stack.** Nav 3's back stack is plain Compose state,
  which is useful when navigation must be driven by a state machine or restored
  explicitly.
